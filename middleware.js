...
async function getAllHash(dbo, address, txDate, type) {
  ...
  try {
    ...
    {info = await dbo
    .collection(address)
    .find({ $and: [{txType:type}, {isActive: true}] }, { projection: { _id: 1 } })
    .toArray();}

    else
    {info = await dbo
      .collection(address)
      .find({ $and: [{date: txDate}, {txType:type}, {isActive: true}] }, { projection: { _id: 1 } })
      .toArray();}
...
async function getAllDisabledHash(dbo, address, txDate, type) {
  var transactionArray = [];
  let info
  try {
    if(txDate.toString() === '0')
    {info = await dbo
    .collection(address)
    .find({ $and: [{txType:type}, {isActive: false}] }, { projection: { _id: 1 } })
    .toArray();}

    else
    {info = await dbo
      .collection(address)
      .find({ $and: [{date: txDate}, {txType:type}, {isActive: false}] }, { projection: { _id: 1 } })
      .toArray();}
...
async function updateSeedProfile(dbo, seed, info) {
  try {
    var myquery = { SEED: seed };
    var newvalues = { $set: { Profile: info } };
    await dbo.collection("SEEDS").updateOne(myquery, newvalues);
...
async function updateSeedPassword(dbo, seed, password) {
  try {
    var myquery = { SEED: seed };
...
async function changePresDescription(dbo, address, txHash, status) {
  try {
    var myquery = { txHash: txHash };
    var newvalues = { $set: { isActive: status } };
    await dbo.collection(address).updateOne(myquery, newvalues);
...
async function getAlphaAddress(dbo, seed) {
  try {
    var result = await dbo
      .collection(seed)
...
async function dropSeed(dbo, seed) {
  try {
    var result = await dbo.collection("SEEDS").findOne({ SEED: seed });
    var res = await dbo.collection("OLD_SEEDS").insertOne(result);
    // console.log("Response to Old Seed ", res)
    // if(res == true)
    await dbo.collection("SEEDS").deleteOne({ SEED: seed });
...
async function getAllOldSeeds(dbo, username, password) {
  var loggedIN = await adminLogin(dbo, username, password);
  if (loggedIN == true) {
    var seedsAray = [];
    try {
